...
namespace Notepad
{
    class VirtualControl
    {
        ...
        public static void ControlSendMessage(string winTitle, VirtualKeyStates Key, bool shift)
        {
            ...
            if ((int)hWnd == 0)
            {
                MessageBox.Show("Unable to find a window with that title");
                return;
            }

            IntPtr child = FindWindowEx(hWnd, new IntPtr(0), "Edit", null);
            ...
            {
                if ((int)child != 0)  // Example Notepad
                {
                    string strKeyToSend = Convert.ToChar((int)Key).ToString();

                    //send key down                    
                    SendMessage(child, 0x000C, 0, strKeyToSend);
                    //sleep 50ms
                    Thread.Sleep(50);
                    //send key up                    
                    SendMessage(child, 0x000C, 0, strKeyToSend);
                }
                else // Example DirectX game like WoW
                {
                    //send key down
                    SendMessage(hWnd, 0x100, (int)Key, 0);
                    //sleep 50ms
